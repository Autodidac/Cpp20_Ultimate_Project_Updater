...
#if defined(_WIN32)
    std::ifstream file(config::SEVEN_ZIP_LOCAL_BINARY());
...
inline bool setup_7zip() {
    ...
    if (!download_file(config::SEVEN_ZIP_EXE_URL(), std::string(installer))) {
...
inline bool extract_archive(const std::string& archive_file, const std::string& destination = ".") {
    ...
    std::string extract_command = "cmd.exe /C \"\"" + config::SEVEN_ZIP_LOCAL_BINARY() + "\" x \"" + archive_file + "\" -o\"" + destination + "\" -y\"";
...
// ⚙ **Setup LLVM + Clang**
    inline bool setup_llvm_clang() {
        ...
        std::string clang_binary = config::LLVM_BIN_PATH() + "/clang++";
#if defined(_WIN32)
        clang_binary += ".exe";
#endif
        ...
        if (!download_file(config::LLVM_SOURCE_URL(), llvm_zip)) {
            std::cerr << "[ERROR] Failed to download LLVM + Clang!\n";
            return false;
        }

        // ✅ First Attempt: Extract using 7-Zip
        std::string extract_command = "\"\"" + config::SEVEN_ZIP_LOCAL_BINARY() + "\" x \"" + llvm_zip + "\" -o\"" + temp_tools_dir + "\" -y\"";
        ...
        if (system(build_command.c_str()) != 0) {
            ...
            if (!download_file(config::LLVM_EXE_URL(), "llvm_installer.exe")) {
...
inline bool setup_ninja() {
    ...
    return download_file(config::NINJA_ZIP_URL(), "ninja.zip") && extract_archive("ninja.zip");
}
...
inline bool generate_ninja_build_file() 
{
    ...
    std::string llvm_bin = config::LLVM_BIN_PATH();

#if defined(_WIN32)


    buildFile << "rule cxx\n"
        << " command = " << llvm_bin << "/clang++ -I include -o" << config::OUTPUT_BINARY() 
        << " -std=c++20 -fuse-ld=lld -D_WIN32 " << config::SOURCE_MAIN_FILE() << "\n"
